...
class TestLoadConfig(unittest.TestCase):
    ...
    @patch(
        "src.loadConfig.os.getenv",
        side_effect=lambda key, default=None: default,
    )
    @patch(
        "src.loadConfig.os.path.exists", return_value=False
    )  # No .toml file exists
    @patch(
        "builtins.open", new_callable=mock_open, read_data="{}"
    )  # Empty JSON config
    @patch(
        "src.loadConfig.toml.load", return_value={}
    ...
    @patch("src.loadConfig.os.path.exists", return_value=True)  # File exists
    @patch(
        "builtins.open",
    ...
    @patch(
        "src.loadConfig.toml.load", return_value={"api_key": "test_key"}
    )  # Mock config data
